...
export default function BookingForm() {
  ...
  const handleBooking = async (e: any) => {
    ...
    try {
      // 1. Check if the slot is already taken in the database
      ...
      if (existing && existing.length > 0) {
        setStatus("❌ Sorry, this time is already booked. Please try another!");
        setLoading(false);
        return;
      }

      // 2. Save to your Supabase logbook using exact column names
  ...
  // PROFESSIONAL SUCCESS CARD VIEW (Matches your original design)
  if (status === "SUCCESS") {
    return (
      <div className="flex justify-center items-center py-20 px-4 bg-white" data-aos="zoom-in">
        <div className="bg-white rounded-3xl shadow-2xl flex flex-col md:flex-row max-w-2xl w-full overflow-hidden border border-gray-100">
          ...
          <div className="p-12 md:w-2/3 flex flex-col items-center justify-center text-center bg-white">
            ...
            <button onClick={() => setStatus('')} className="text-[#c5a059] hover:underline font-bold uppercase tracking-widest text-xs">
              Book another
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleBooking} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input name="date" type="date" required className="w-full p-4 bg-gray-50 border-none rounded-xl outline-none" />
            </div>
            <div className="space-y-1">
