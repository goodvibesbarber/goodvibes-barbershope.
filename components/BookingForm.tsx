  ...
  const handleBooking = async (e: any) => {
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        setStatus("❌ Sorry, this time was just taken!");
        setLoading(false);
        return;
      }
        ...
        <div className="bg-white rounded-3xl shadow-2xl flex flex-col md:flex-row max-w-2xl w-full overflow-hidden border border-gray-100">
          <div className="bg-[#1a1a1a] text-white p-10 md:w-1/3 flex flex-col justify-between">
            <div>
              ...
              <p className="text-gray-400 text-[11px] leading-relaxed">Secure your spot with Simonyo.</p>
            </div>
            <div className="mt-8">
      ...
      <div className="max-w-xl mx-auto bg-white p-10 rounded-[2rem] shadow-xl border border-gray-100" data-aos="fade-up">
        ...
        <form onSubmit={handleBooking} className="space-y-4">
          ...
          {status && status !== "SUCCESS" && (
            <div className="mt-4 p-3 bg-red-50 text-red-600 rounded-lg text-center font-bold text-sm">
              {status}
            </div>
          )}
        </form>
